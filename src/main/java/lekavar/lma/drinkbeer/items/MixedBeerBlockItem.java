...
public class MixedBeerBlockItem extends BeerBlockItem {
    public MixedBeerBlockItem(ResourceKey<Item> key, Block block) {
        super(block, new Item.Properties().useBlockDescriptionPrefix().setId(key).stacksTo(1)
                // коктейль — питьё: звук с 1.21.2 живёт в компоненте Consumable,
                // апстримовский BeerBlockItem#getEatingSound больше не вызывается
                .food(new FoodProperties.Builder().alwaysEdible().build(), BeerMugItem.drinkWithEffect(null))
